...
from dao.reservation_dao import get_guide_tour_metrics,submit_tour_attendance_report
from PIL import Image
from datetime import datetime

guides_bp = Blueprint('guides_route',__name__)
...
def process_and_save_image(file_storage, folder_path, target_height=400, idx=None):
    ...
    left = int(new_width / 2 - target_height / 2)
    top = 0
    right = int(new_width / 2 + target_height / 2)
...
def create_new_tour():
    ...
    if request.method == 'POST':
        ...
        if len(valid_stops) < 4:
            ...
            return redirect(url_for('guides_route.create_new_tour'))
    ...
    return render_template('tour_form.html', action="Create", languages=current_user.languages)
...
def edit_tour(tour_id):
    ...
    if request.method == 'POST':
        ...
        if locked:
            ...
            if len(valid_stops) < 4:
                ...
                return redirect(url_for('guides_route.edit_tour', tour_id=tour_id))
    ...
    return render_template('tour_form.html', action="Edit", tour=tour, sched_map=sched_map, locked=locked, languages=current_user.languages)
...
def report_tour(tour_id, date_str):
    ...
    return render_template('report.html', tour=tour, date_str=date_str)
    ...
    if request.method == 'POST':
        ...
        if new_password:
            if new_password == confirm_password:
                ...
                return redirect(url_for('guides_route.profile'))
